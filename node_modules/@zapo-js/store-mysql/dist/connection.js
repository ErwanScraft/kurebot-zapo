...
exports.ensureMysqlMigrations = ensureMysqlMigrations;
...
async function ensureMysqlMigrations(pool, domains, tablePrefix = '') {
    (0, helpers_1.assertSafeTablePrefix)(tablePrefix);
    ...
    `);
    const applied = new Set((0, helpers_1.queryRows)(await pool.execute(`SELECT name FROM \`${t('_migrations')}\``)).map((r) => r.name));
    for (const migration of pending) {
        ...
        try {
            ...
            await conn.execute(`INSERT IGNORE INTO \`${t('_migrations')}\` (name, applied_at) VALUES (?, ?)`, [migration.name, Date.now()]);
            await conn.commit();
        }
        catch (err) {
            await conn.rollback();
