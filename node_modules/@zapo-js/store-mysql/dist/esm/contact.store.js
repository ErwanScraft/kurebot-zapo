...
export class WaContactMysqlStore extends BaseMysqlStore {
    constructor(options) {
    ...
    async getByJid(jid) {
        await this.ensureReady();
        const row = queryFirst(await this.pool.execute(`SELECT jid, display_name, push_name, lid,
                    phone_number, last_updated_ms
    ...
    async deleteByJid(jid) {
        await this.ensureReady();
        return affectedRows(await this.pool.execute(`DELETE FROM ${this.t('mailbox_contacts')}
             WHERE session_id = ? AND jid = ?`, [this.sessionId, jid]));
    }
    async clear() {
        await this.ensureReady();
