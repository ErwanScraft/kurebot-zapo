...
import { assertSafeTablePrefix, queryRows } from './helpers.js';
const MIGRATIONS = [
    {
...
export async function ensureMysqlMigrations(pool, domains, tablePrefix = '') {
    ...
    `);
    const applied = new Set(queryRows(await pool.execute(`SELECT name FROM \`${t('_migrations')}\``)).map((r) => r.name));
