...
import { affectedRows, queryFirst, queryRows, safeLimit } from './helpers.js';
export class WaThreadMysqlStore extends BaseMysqlStore {
    constructor(options) {
        super(options, ['mailbox']);
    }
    async upsert(record) {
        await this.ensureReady();
        await this.pool.execute(`INSERT INTO ${this.t('mailbox_threads')} (
    ...
    async upsertBatch(records) {
        ...
        const runChunk = async (executor, chunk) => {
            ...
            for (const record of chunk) {
                params.push(this.sessionId, record.jid, record.name ?? null, record.unreadCount ?? null, record.archived === undefined ? null : record.archived ? 1 : 0, record.pinned ?? null, record.muteEndMs ?? null, record.markedAsUnread === undefined ? null : record.markedAsUnread ? 1 : 0, record.ephemeralExpiration ?? null, record.ephemeralSettingTimestamp ?? null);
            }
            await executor.execute(`INSERT INTO ${this.t('mailbox_threads')} (
                    session_id, jid, name, unread_count, archived, pinned,
                    mute_end_ms, marked_as_unread, ephemeral_expiration, ephemeral_setting_timestamp
                ) VALUES ${placeholders}
                ON DUPLICATE KEY UPDATE
                    ...
                    marked_as_unread = COALESCE(VALUES(marked_as_unread), marked_as_unread),
                    ephemeral_expiration = COALESCE(VALUES(ephemeral_expiration), ephemeral_expiration),
                    ephemeral_setting_timestamp = COALESCE(VALUES(ephemeral_setting_timestamp), ephemeral_setting_timestamp)`, params);
        };
    ...
    async getByJid(jid) {
        await this.ensureReady();
        const row = queryFirst(await this.pool.execute(`SELECT jid, name, unread_count, archived, pinned,
                    mute_end_ms, marked_as_unread, ephemeral_expiration, ephemeral_setting_timestamp
    ...
    async list(limit) {
        ...
        return queryRows(await this.pool.execute(`SELECT jid, name, unread_count, archived, pinned,
                    mute_end_ms, marked_as_unread, ephemeral_expiration, ephemeral_setting_timestamp
             FROM ${this.t('mailbox_threads')}
             WHERE session_id = ?
             LIMIT ${resolved}`, [this.sessionId])).map(rowToRecord);
    }
    async deleteByJid(jid) {
        await this.ensureReady();
        return affectedRows(await this.pool.execute(`DELETE FROM ${this.t('mailbox_threads')}
             WHERE session_id = ? AND jid = ?`, [this.sessionId, jid]));
    }
    async clear() {
        await this.ensureReady();
...
function rowToRecord(row) {
    return {
        ...
        markedAsUnread: row.marked_as_unread === null ? undefined : Number(row.marked_as_unread) === 1,
        ephemeralExpiration: row.ephemeral_expiration !== null ? Number(row.ephemeral_expiration) : undefined,
        ephemeralSettingTimestamp: row.ephemeral_setting_timestamp !== null
