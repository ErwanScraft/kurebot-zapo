...
import { queryFirst, toBytes } from './helpers.js';
export class WaSignalMysqlStore extends BaseMysqlStore {
    constructor(options) {
    ...
    async getRegistrationInfo() {
        ...
        const row = queryFirst(await this.pool.execute(`SELECT registration_id, identity_pub_key, identity_priv_key
    ...
    async getSignedPreKey() {
        await this.ensureReady();
        const row = queryFirst(await this.pool.execute(`SELECT key_id, pub_key, priv_key, signature, uploaded
    ...
    async getMeta(conn) {
        await this.ensureMetaRow(conn);
        const row = queryFirst(await conn.execute(`SELECT signed_prekey_rotation_ts
