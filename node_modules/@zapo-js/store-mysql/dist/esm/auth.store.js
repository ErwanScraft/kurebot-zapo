...
import { queryFirst, toBytes, toBytesOrNull } from './helpers.js';
export class WaAuthMysqlStore extends BaseMysqlStore {
    constructor(options) {
        super(options, ['auth']);
    }
    async load() {
        await this.ensureReady();
        const row = queryFirst(await this.pool.execute(`SELECT noise_pub_key, noise_priv_key, registration_id,
        ...
        return {
            ...
            signedIdentity: signedIdentityBytes
                ...
                : undefined,
            meJid: row.me_jid ?? undefined,
            meLid: row.me_lid ?? undefined,
            meDisplayName: row.me_display_name ?? undefined,
            ...
            serverHasPreKeys: row.server_has_prekeys === null ? undefined : Number(row.server_has_prekeys) === 1,
            ...
            abPropsVersion: row.ab_props_version !== null ? Number(row.ab_props_version) : undefined,
            connectionLocation: row.connection_location ?? undefined,
            accountCreationTs: row.account_creation_ts !== null ? Number(row.account_creation_ts) : undefined,
    ...
    async save(credentials) {
        await this.ensureReady();
        await this.pool.execute(`INSERT INTO ${this.t('auth_credentials')} (
