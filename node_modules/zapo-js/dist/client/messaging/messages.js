...
const conn_1 = require("../../media/transfer/conn");
const builder_1 = require("../../message/addons/link-preview/builder");
...
function targetMessageKey(remoteJid, key) {
    const inGroup = (0, jid_1.isGroupJid)(remoteJid) || (0, jid_1.isBroadcastJid)(remoteJid);
...
function buildReactionMessage(content, serverClock, ctx) {
    ...
    return {
        reactionMessage: {
            key: targetMessageKey(ctx.to, (0, content_1.resolveMessageTarget)(content.target)),
            text: content.emoji,
...
function buildRevokeMessage(content, ctx) {
    ...
    return {
        protocolMessage: {
            type: _proto_1.proto.Message.ProtocolMessage.Type.REVOKE,
            key: targetMessageKey(ctx.to, (0, content_1.resolveMessageTarget)(content.target))
        }
    };
}
...
function buildPinMessage(content, serverClock, ctx) {
    ...
    return {
        pinInChatMessage: {
            key: targetMessageKey(ctx.to, (0, content_1.resolveMessageTarget)(content.target)),
            type: isPin
                ? _proto_1.proto.Message.PinInChatMessage.Type.PIN_FOR_ALL
                : _proto_1.proto.Message.PinInChatMessage.Type.UNPIN_FOR_ALL,
            senderTimestampMs: content.senderTimestampMs ?? serverClock.nowMs()
        },
        messageContextInfo: {
    ...
    return {
        keepInChatMessage: {
            ...
            keepType: content.type === 'keep'
                ? _proto_1.proto.KeepType.KEEP_FOR_ALL
                : _proto_1.proto.KeepType.UNDO_KEEP_FOR_ALL,
...
const EVENT_RESPONSE_ENUM = {
    going: _proto_1.proto.Message.EventResponseMessage.EventResponseType.GOING,
    not_going: _proto_1.proto.Message.EventResponseMessage.EventResponseType.NOT_GOING,
    maybe: _proto_1.proto.Message.EventResponseMessage.EventResponseType.MAYBE
};
async function encryptAddonForOutgoing(input) {
    ...
    const iv = await (0, _crypto_1.randomBytesAsync)(12);
    const additionalData = (0, addon_crypto_1.shouldUseAddonAdditionalData)(input.modificationType)
        ? (0, addon_crypto_1.buildAddonAdditionalData)(input.parentStanzaId, input.modificationSender)
        : undefined;
    const encPayload = (0, _crypto_1.aesGcmEncrypt)(secret, iv, input.payload, additionalData);
...
async function buildPollVoteMessage(content, serverClock, ctx) {
    ...
    const selectedOptions = content.selectedOptionNames.map((name) => (0, _crypto_1.sha256)(bytes_1.TEXT_ENCODER.encode(name)));
    const payload = _proto_1.proto.Message.PollVoteMessage.encode({ selectedOptions }).finish();
    const { encPayload, encIv } = await encryptAddonForOutgoing({
        ...
        parentMsgOriginalSender: (0, jid_1.toUserJid)(content.poll.authorJid),
        modificationSender: (0, jid_1.toUserJid)(ctx.meJid),
...
async function buildEventResponseMessage(content, serverClock, ctx) {
    ...
    const payload = _proto_1.proto.Message.EventResponseMessage.encode(responseProto).finish();
    const { encPayload, encIv } = await encryptAddonForOutgoing({
        messageSecret: content.event.messageSecret,
        parentStanzaId: content.event.id,
        parentMsgOriginalSender: (0, jid_1.toUserJid)(content.event.authorJid),
        modificationSender: (0, jid_1.toUserJid)(ctx.meJid),
...
async function buildMediaMessageContent(options, content, ctx) {
    if (typeof content === 'string') {
        return { message: { conversation: content } };
    }
    if ((0, content_1.isSendTextMessage)(content)) {
        if (options.linkPreviewResolver) {
            try {
                const preview = await options.linkPreviewResolver(content);
                if (preview !== null) {
                    return {
                        message: (0, builder_1.buildExtendedTextWithPreview)(content.text, preview.resolved, preview.thumbnailFields)
                    };
                }
            }
            catch (error) {
...
async function getMediaConn(options, forceRefresh = false) {
...
async function buildMediaMessage(options, content) {
    ...
    const processorCtx = {
        logger: (0, media_1.getScopedMediaLogger)(options.logger)
    };
    if ((0, media_1.shouldNormalizeVoiceNote)(options.media, content)) {
        const sourceInput = content.media instanceof ArrayBuffer ? (0, bytes_1.toBytesView)(content.media) : content.media;
        try {
    ...
    const needsMimetypeDetection = !content.mimetype &&
        ...
        (0, media_1.isReadableStream)(content.media);
...
function parseUploadResponse(body, status) {
    (0, media_1.assertMediaUploadStatus)(status, 'media upload');
    const parsed = (0, media_1.parseMediaUploadJsonBody)(body, 'media upload');
...
async function buildStickerPackMediaMessage(options, content) {
    (0, sticker_pack_2.validateStickerPackInput)(content);
    ...
    return {
        ...
        message: {
            stickerPackMessage: {
                ...
                stickers: (0, sticker_pack_2.toStickerPackProtoStickers)(content),
                fileLength: bundle.fileLength,
                fileSha256: bundle.fileSha256,
                ...
                stickerPackOrigin: _proto_1.proto.Message.StickerPackMessage.StickerPackOrigin.USER_CREATED,
                imageDataHash: (0, bytes_1.bytesToBase64)(cover.fileSha256),
