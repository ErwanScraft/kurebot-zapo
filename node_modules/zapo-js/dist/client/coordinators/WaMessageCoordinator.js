...
exports.WaMessageCoordinator = void 0;
...
function oldestTimestampSeconds(messages) {
    let oldest;
    for (let index = 0; index < messages.length; index += 1) {
        const timestamp = (0, primitives_1.longToNumber)(messages[index].messageTimestamp);
...
function parseReachoutTimelockMexResponse(data) {
    ...
    return {
        ...
        enforcementType: (0, coercion_1.tryAsString)(root?.enforcement_type),
        enforcementEndsAt: (0, coercion_1.tryAsNumber)(root?.time_enforcement_ends)
    };
}
function parseMessageCappingMexResponse(data) {
    const root = data?.xwa2_message_capping_info;
    return {
        totalQuota: (0, coercion_1.tryAsNumber)(root?.total_quota),
        usedQuota: (0, coercion_1.tryAsNumber)(root?.used_quota),
        cycleStartAt: (0, coercion_1.tryAsNumber)(root?.cycle_start_timestamp),
        cycleEndAt: (0, coercion_1.tryAsNumber)(root?.cycle_end_timestamp),
        serverSentAt: (0, coercion_1.tryAsNumber)(root?.server_sent_timestamp),
        oteStatus: (0, coercion_1.tryAsString)(root?.ote_status),
        mvStatus: (0, coercion_1.tryAsString)(root?.mv_status),
...
class WaMessageCoordinator {
    ...
    async requestHistorySync(input) {
        const chatJid = (0, jid_1.normalizeRecipientJid)(input.chatJid);
        const onDemandMsgCount = (0, coercion_1.resolveOptionalPositive)(input.count, 'count');
        ...
        return this.peerDataOperation.send(_proto_1.proto.Message.PeerDataOperationRequestType.HISTORY_SYNC_ON_DEMAND, { historySyncOnDemandRequest });
    }
    /**
     ...
     * ```
     */
    async shareGroupHistory(groupJid, input) {
        const normalizedGroupJid = (0, jid_1.normalizeRecipientJid)(groupJid);
        if (!(0, jid_1.isGroupJid)(normalizedGroupJid)) {
        ...
        const messageLimit = (0, coercion_1.resolvePositive)(input.count, this.getAbPropNumber('group_history_message_count_limit'), 'count');
        if (!this.isGroupHistorySendEnabled()) {
            throw new Error('shareGroupHistory is disabled for this account (group_history_send is off)');
        }
        const audience = await this.messageDispatch.resolveGroupHistoryAudience(normalizedGroupJid, input.toJids);
        ...
        if (audience.unknownJids.length > 0) {
            throw new Error(`shareGroupHistory recipients are not members of the group (addressing mode: ${audience.addressingMode}): ${audience.unknownJids.join(', ')}`);
        }
        ...
        const { compressed } = await (0, group_history_1.encodeGroupHistoryBundle)(messages, input.outOfWindowPinnedMessages);
        const upload = await (0, messages_1.uploadMedia)(this.mediaUploadOptions, {
            source: compressed,
            cryptoType: 'group-history',
            uploadPath: constants_1.MEDIA_UPLOAD_PATHS['group-history'],
        ...
        const oldestInBundle = oldestPin === undefined
            ? oldestInWindow
        ...
        let noticeMessageId;
        try {
            noticeMessageId = (await this.send(normalizedGroupJid, {
     ...
     */
    async loadGroupHistoryMessages(groupJid, limit, sinceMs) {
        ...
        for (let index = 0; index < records.length; index += 1) {
            ...
            messages[messages.length] = {
                ...
                message: _proto_1.proto.Message.decode(record.messageBytes),
                messageTimestamp: Math.floor((record.timestampMs ?? 0) / 1000)
            };
        }
        return messages;
    }
    ...
    async getReachoutTimelock() {
        const data = await (0, client_1.runMexQuery)(this.mexSocket, 'FetchReachoutTimelock', {});
    ...
    async getNewChatMessageCapping(type = 'INDIVIDUAL_NEW_CHAT_THREAD') {
        const data = await (0, client_1.runMexQuery)(this.mexSocket, 'FetchNewChatMessageCappingInfo', {
            input: { type }
        });
    ...
    async sendReceipt(first, second, third) {
        ...
        const targets = events.map((event) => {
            ...
            return {
                ...
                id: event.key.id,
                senderJid: senderJid
                    ? (0, jid_1.applyDeviceToJid)(senderJid, event.key.senderDevice)
                    : undefined,
                isGroupChat: event.key.isGroup,
                isBroadcastChat: event.key.isBroadcast
            };
        });
        for (const group of (0, receipt_1.aggregateReceiptTargets)(targets)) {
    ...
    async downloadToFile(source, filePath, options = {}) {
        ...
        await (0, promises_1.pipeline)(stream, (0, node_fs_1.createWriteStream)(filePath));
    }
    /**
    ...
    async tryDecryptAddon(event) {
        ...
        if (!parentEntry) {
            ...
            if ((0, content_1.unwrapMessage)(message).secretEncryptedMessage) {
...
exports.WaMessageCoordinator = WaMessageCoordinator;
