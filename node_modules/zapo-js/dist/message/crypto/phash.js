...
exports.computePhashV2 = computePhashV2;
...
function writeCanonicalUtf8(out, start, jid) {
    ...
    if (isCUs) {
        off = writeAscii(out, off, constants_1.WA_DEFAULTS.HOST_DOMAIN, 0, constants_1.WA_DEFAULTS.HOST_DOMAIN.length);
    }
    else {
...
function writeAscii(out, outOff, str, start, end) {
    for (let i = start; i < end; i += 1) {
        out[outOff + (i - start)] = str.charCodeAt(i);
    }
    return outOff + (end - start);
}
function writeUintAscii(out, off, value) {
    if (value === 0) {
        out[off] = CHAR_ZERO;
        return off + 1;
    }
...
function compareScratchSlice(scratch, offsets, a, b) {
