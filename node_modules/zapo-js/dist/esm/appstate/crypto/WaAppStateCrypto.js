...
import { aesCbcDecrypt, aesCbcEncrypt, hmacSha256Sign, hmacSha512Sign } from '../../crypto/core/primitives.js';
...
export class WaAppStateCrypto {
    ...
    deriveKeys(keyData) {
        ...
        const derived = hkdf(keyData, null, WA_APP_STATE_KDF_INFO.MUTATION_KEYS, APP_STATE_DERIVED_KEY_LENGTH);
        const keys = {
            indexHmacKey: derived.subarray(0, APP_STATE_DERIVED_INDEX_KEY_END),
            valueEncryptionAesKey: derived.subarray(APP_STATE_DERIVED_INDEX_KEY_END, APP_STATE_DERIVED_VALUE_ENCRYPTION_KEY_END),
            valueMacHmacKey: derived.subarray(APP_STATE_DERIVED_VALUE_ENCRYPTION_KEY_END, APP_STATE_DERIVED_VALUE_MAC_KEY_END),
            snapshotMacHmacKey: derived.subarray(APP_STATE_DERIVED_VALUE_MAC_KEY_END, APP_STATE_DERIVED_SNAPSHOT_MAC_KEY_END),
            patchMacHmacKey: derived.subarray(APP_STATE_DERIVED_SNAPSHOT_MAC_KEY_END, APP_STATE_DERIVED_PATCH_MAC_KEY_END)
        };
    ...
    ltHashApply(base, values, combine) {
        ...
        const expandedValues = values.map((value) => hkdf(value, null, WA_APP_STATE_KDF_INFO.PATCH_INTEGRITY, APP_STATE_EMPTY_LT_HASH.byteLength));
    ...
    generateAssociatedData(operation, keyId) {
        if (operation !== proto.SyncdMutation.SyncdOperation.SET &&
    ...
    generateValueMac(valueMacHmacKey, associatedData, iv, cipherText) {
        ...
        const full = hmacSha512Sign(valueMacHmacKey, concatBytes([associatedData, iv, cipherText, octetLength]));
