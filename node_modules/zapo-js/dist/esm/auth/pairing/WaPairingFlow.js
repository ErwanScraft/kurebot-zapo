...
export class WaPairingFlow {
    ...
    async fetchPairingCountryCodeIso() {
        ...
        const response = await this.opts.socket.query(buildGetCountryCodeRequestNode(), WA_DEFAULTS.IQ_TIMEOUT_MS);
    ...
    async handleCompanionRegRefreshNotification(node) {
        const hasExpectedChild = hasNodeChild(node, WA_SIGNALING.COMPANION_REG_REFRESH_NOTIFICATION) ||
    ...
    async handlePairSuccess(iqNode, pairSuccessNode) {
        ...
        const wrappedIdentity = proto.ADVSignedDeviceIdentityHMAC.decode(decodeNodeContentUtf8OrBytes(deviceIdentityNode.content, 'pair-success.device-identity'));
        const wrappedDetails = decodeProtoBytes(wrappedIdentity.details, 'ADVSignedDeviceIdentityHMAC.details');
    ...
    async handlePrimaryHello(linkCodeNode) {
        ...
        const finish = await completeCompanionFinish({
            ...
            wrappedPrimaryEphemeralPub: decodeNodeContentUtf8OrBytes(wrappedPrimaryNode.content, 'primary_hello.link_code_pairing_wrapped_primary_ephemeral_pub'),
            primaryIdentityPub: decodeNodeContentUtf8OrBytes(primaryIdentityNode.content, 'primary_hello.primary_identity_pub'),
