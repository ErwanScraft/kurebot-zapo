import { WA_MESSAGE_TAGS, WA_MESSAGE_TYPES, WA_NODE_TAGS } from '../../../protocol/constants.js';
...
export function buildButtonAddonNode(kind) {
    const inner = kind === 'list'
        ? {
...
export function buildDirectMessageFanoutNode(input) {
    ...
    const content = [
        {
            ...
            attrs: {},
            content: input.participants.map((p) => buildEncryptedToNode(p, input.mediatype, input.decryptFail))
        }
    ];
...
export function buildGroupSenderKeyMessageNode(input) {
    const attrs = buildMessageAttrs(input);
    const content = [];
    if (input.participants.length > 0) {
        content.push({
...
export function buildGroupRetryMessageNode(input) {
    const content = [];
    if (input.metaNode) {
...
export function buildMetaNode(attrs) {
    return {
        tag: 'meta',
        attrs,
        content: undefined
