...
export async function decryptMsg(session, parsed, onPrevSessionDecryptError) {
    if (!session) {
...
export async function decryptMsgFromSession(session, message) {
    ...
    let updatedSession;
    if (recvChainIndex === -1) {
        const recvRatchet = await calculateRatchet(session.rootKey, session.sendChain.ratchetKey, ratchetPubKey);
