...
exports.decodeBinaryNodeStanza = decodeBinaryNodeStanza;
const node_util_1 = require("node:util");
const node_zlib_1 = require("node:zlib");
const constants_1 = require("../../protocol/constants");
const constants_2 = require("../binary/constants");
...
function readBinary(reader, token) {
    ...
    if (token === constants_2.BINARY_20) {
        const length = ((reader.readUint8() & 0x0f) << 16) | (reader.readUint8() << 8) | reader.readUint8();
        return reader.readBytes(length);
    }
    if (token === constants_2.BINARY_32) {
        return reader.readBytes(reader.readUint32());
    }
    throw new Error(`invalid binary token ${token}`);
}
function decodeTokenString(token, reader) {
    if (token > 0 && token <= tokens_1.SINGLE_BYTE_TOKENS.length) {
        return tokens_1.SINGLE_BYTE_TOKENS[token - 1];
    }
    if (token >= constants_2.DICTIONARY_0 && token <= constants_2.DICTIONARY_3) {
        const dictionaryIndex = token - constants_2.DICTIONARY_0;
        const index = reader.readUint8();
        const dictionary = tokens_1.DICTIONARIES[dictionaryIndex];
        if (!dictionary || index >= dictionary.length) {
            throw new Error(`invalid dictionary token ${dictionaryIndex}:${index}`);
        }
        return dictionary[index];
    }
    if (token === constants_2.NIBBLE_8) {
        return parsePacked(reader, constants_2.NIBBLE_ALPHABET);
    }
    if (token === constants_2.HEX_8) {
        return parsePacked(reader, constants_2.HEX_ALPHABET);
    }
    if (token === constants_2.BINARY_8 || token === constants_2.BINARY_20 || token === constants_2.BINARY_32) {
        return bytes_1.TEXT_DECODER.decode(readBinary(reader, token));
    }
    throw new Error(`unsupported string token ${token}`);
}
function decodeJidPair(reader) {
    const userToken = reader.readUint8();
    const user = userToken === constants_2.LIST_EMPTY ? '' : decodeTokenString(userToken, reader);
...
function decodeJidInterop(reader) {
    const user = decodeTokenString(reader.readUint8(), reader);
...
function decodeJidFb(reader) {
    const user = decodeTokenString(reader.readUint8(), reader);
    const device = reader.readUint16();
    decodeTokenString(reader.readUint8(), reader);
    return `${user}:${device}@msgr`;
}
function decodeValue(reader, token, forAttr) {
    if (token === constants_2.LIST_EMPTY) {
        return null;
    }
    if (token === constants_2.JID_PAIR) {
        return decodeJidPair(reader);
    }
    if (token === constants_2.JID_U) {
        return decodeJidU(reader);
    }
    if (token === constants_2.JID_INTEROP) {
        return decodeJidInterop(reader);
    }
    if (token === constants_2.JID_FB) {
        return decodeJidFb(reader);
    }
    if (token === constants_2.BINARY_8 || token === constants_2.BINARY_20 || token === constants_2.BINARY_32) {
        const binary = readBinary(reader, token);
        return forAttr ? bytes_1.TEXT_DECODER.decode(binary) : binary;
    }
    const value = decodeTokenString(token, reader);
    return value;
}
function decodeNodeInternal(reader) {
    const listType = reader.readUint8();
    let listSize = 0;
    if (listType === constants_2.LIST_8) {
        listSize = reader.readUint8();
    }
    else if (listType === constants_2.LIST_16) {
    ...
    if (remaining === 1) {
        ...
        if (contentToken === constants_2.LIST_EMPTY) {
            content = undefined;
        }
        else if (contentToken === constants_2.LIST_8 || contentToken === constants_2.LIST_16) {
            const childrenCount = contentToken === constants_2.LIST_8 ? reader.readUint8() : reader.readUint16();
            ...
            if (value === null) {
                ...
                content = bytes_1.TEXT_ENCODER.encode(value);
            }
            else {
                content = value;
            }
...
async function inflateCompressedStanza(data) {
    try {
        return (0, bytes_1.toBytesView)(await unzipAsync(data));
    }
    catch {
        try {
            return (0, bytes_1.toBytesView)(await inflateRawAsync(data));
        }
        catch (rawError) {
...
async function decodeBinaryNodeStanza(stanza) {
    ...
    if (flag === constants_2.STREAM_END && reader.getRemaining() === 0) {
